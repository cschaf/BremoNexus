const propertyData = [
  {
    id: 1,
    image: 'images/001_property.png',
    name: 'Moderne Eigentumswohnung in Findorff',
    price: '320.000 €',
    description: 'Diese helle und hochwertig renovierte 3-Zimmer-Wohnung befindet sich in einer begehrten Lage in Bremen-Findorff...',
    features: [
      { icon: 'fas fa-ruler-combined', text: '85 m²' },
      { icon: 'fas fa-bed', text: '3 Zimmer' },
      { icon: 'fas fa-calendar', text: 'Bj. 2010' }
    ]
  },
  {
    id: 2,
    image: 'images/002_property.png',
    name: 'Einfamilienhaus in Oberneuland mit Garten',
    price: '650.000 €',
    description: 'Ein Traum für Familien: Dieses gepflegte Einfamilienhaus liegt im grünen und ruhigen Stadtteil Oberneuland...',
    features: [
      { icon: 'fas fa-ruler-combined', text: '160 m²' },
      { icon: 'fas fa-bed', text: '5 Zimmer' },
      { icon: 'fas fa-calendar', text: 'Bj. 1998' }
    ]
  },
  {
    id: 3,
    image: 'images/003_property.png',
    name: 'Repräsentative Bürofläche in der Innenstadt',
    price: '12 €/m² zzgl. NK',
    description: 'Moderne und flexible Bürofläche in zentraler Bremer Innenstadtlage. Die Auswahl der Räume ist groß...',
    features: [
      { icon: 'fas fa-ruler-combined', text: '200 m²' },
      { icon: 'fas fa-door-open', text: '6 Räume' },
      { icon: 'fas fa-calendar', text: 'Bj. 2015' }
    ]
  },
  {
    id: 4,
    image: 'images/004_property.png',
    name: 'Gemütliches Studio-Apartment in Neustadt',
    price: '180.000 €',
    description: 'Perfekt für Singles oder Paare: Dieses charmante Studio-Apartment bietet modernen Komfort auf kleinem Raum...',
    features: [
      { icon: 'fas fa-ruler-combined', text: '40 m²' },
      { icon: 'fas fa-bed', text: '1 Zimmer' },
      { icon: 'fas fa-calendar', text: 'Bj. 2018' }
    ]
  },
  {
    id: 5,
    image: 'images/005_property.png',
    name: 'Großzügiges Loft mit Industriecharme in Überseestadt',
    price: '450.000 €',
    description: 'Einzigartiges Loft mit hohen Decken und großen Fensterflächen in der dynamischen Überseestadt...',
    features: [
      { icon: 'fas fa-ruler-combined', text: '120 m²' },
      { icon: 'fas fa-bed', text: '2 Zimmer' },
      { icon: 'fas fa-bath', text: '2 Bäder' },
      { icon: 'fas fa-calendar', text: 'Bj. 2005 (saniert)' }
    ]
  },
  {
    id: 6,
    image: 'images/006_property.png',
    name: 'Büroetage mit Weitblick im Technologiepark',
    price: '15 €/m² zzgl. NK',
    description: 'Helle und moderne Büroetage in einem repräsentativen Gebäude im Technologiepark, ideal für innovative Unternehmen...',
    features: [
      { icon: 'fas fa-ruler-combined', text: '350 m²' },
      { icon: 'fas fa-door-open', text: '10 Räume' },
      { icon: 'fas fa-elevator', text: 'Aufzug vorhanden' },
      { icon: 'fas fa-parking', text: 'Tiefgarage' }
    ]
  }
];
